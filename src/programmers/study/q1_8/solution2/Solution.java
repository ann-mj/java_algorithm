package programmers.study.q1_8.solution2;
...
class Solution {
    ...
    public String[] solution(String[] strings, int n) {
        String[] answer = Arrays.asList(strings)
                .stream().map(a -> new Word(a, n)).sorted().collect(Collectors.toList())
                .stream().map(word -> word.getS()).collect(Collectors.toList()).toArray(String[]::new);
        return answer;
    }
}